...
import { Box, Typography, Grid, Button, Stack, CircularProgress, FormControlLabel, Switch } from '@mui/material';
//Internal App
  ...
  const onAction = (row: RowTable, action: number) => {
    switch (action) {
      ...
      case 2:
        ...
        break;
      case 3:
        console.log('Execute function', row);
        break;
      default:
        break;
    }
      ...
      <Box sx={{ m: 5 }} component="form" onSubmit={handleSubmit(onSubmit)}>
        ...
        <Grid container columns={3} spacing={2}>
          <Grid item xs={1}>
            ...
            <FormControlLabel
              control={<Switch checked={isByService} onChange={onChangeByService} />}
              label="Paginación"
            />
            <Stack direction="row" spacing={2}>
              <Button
              ...
              <Button variant="contained" onClick={() => serviceGetMovements()} disabled={isByService} fullWidth>
                {loading && <CircularProgress color="secondary" size={20} />}
          ...
          <Grid item xs={3}>
            {loading && movements.length === 0 && <CircularProgress color="secondary" size={20} />}
            {movements.length > 0 && (
              <Table
                data={movements}
                columns={columns}
